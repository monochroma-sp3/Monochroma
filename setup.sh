...
# ---------------------------------------------------------------------------
# 3. Node.js + pnpm (for the Feishin web build)
# ---------------------------------------------------------------------------
if [ -d "$REPO_DIR/feishin" ]; then
  ...
  if node_satisfies_min; then
    ...
    warn "Node is still older than v$NODE_MIN_VERSION — the Feishin build below will fail."
  fi
...
# ---------------------------------------------------------------------------
# 5. Build the Feishin web UI
# ---------------------------------------------------------------------------
if [ -d "$REPO_DIR/feishin" ] && command -v pnpm >/dev/null 2>&1; then
  step "Building the Feishin web UI"
  ( cd feishin && pnpm install --frozen-lockfile 2>/dev/null || pnpm install )
  ( cd feishin && pnpm build:web )
  if [ -d "$REPO_DIR/feishin/out/web" ]; then
    ok "Feishin web build at feishin/out/web"
  else
    warn "Expected feishin/out/web not found — check the build output above."
  fi
else
  warn "Skipping Feishin build (missing feishin/ or pnpm)."
fi

# ---------------------------------------------------------------------------
# 6. hifi-api (Tidal audio backend)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 7. Playlist transferer (transferer/, playlist import microservice)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 8. Configuration (navidrome.toml + .env.local from templates)
# ---------------------------------------------------------------------------
step "Configuration"
