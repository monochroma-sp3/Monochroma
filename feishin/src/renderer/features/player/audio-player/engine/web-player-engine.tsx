        ...
        <div id="web-player-engine" style={{ display: 'none' }}>
            <ReactPlayerComponent
                ...
                onEnded={
                    src1 && !loopPlayer1
                        ? () => {
                              // Ignore the EMPTY_SOURCE silent placeholder (~0.072s) firing
                              // `ended` during src transitions — only advance on a real track
                              // finishing. Without this the queue skips every track.
                              const internal = player1Ref.current?.getInternalPlayer();
                              if (internal?.duration && internal.duration < 1) return;
                              onEndedPlayer1();
                          }
                        : undefined
                }
                ...
                onEnded={
                    src2 && !loopPlayer2
                        ? () => {
                              ...
                              // `ended` during src transitions — only advance on a real track
                              // finishing. Without this the queue skips every track.
                              const internal = player2Ref.current?.getInternalPlayer();
                              if (internal?.duration && internal.duration < 1) return;
                              onEndedPlayer2();
                          }
                        : undefined
