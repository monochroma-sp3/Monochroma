...
export const WaveSurferPlayerEngine = (props: WaveSurferPlayerEngineProps) => {
    ...
    useEffect(() => {
        ...
        const handleEnded = () => {
            // Ignore the EMPTY_SOURCE silent placeholder (~0.072s) firing `finish`
            // during src transitions — only advance on a real track finishing.
            if (wavesurfer1.getDuration() < 1) return;
            onEndedPlayer1();
        };
    ...
    useEffect(() => {
        ...
        const handleEnded = () => {
            // Ignore the EMPTY_SOURCE silent placeholder (~0.072s) firing `finish`
            // during src transitions — only advance on a real track finishing.
            if (wavesurfer2.getDuration() < 1) return;
            onEndedPlayer2();
        };
