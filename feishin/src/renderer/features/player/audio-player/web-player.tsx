...
import { usePrewarm } from '/@/renderer/features/player/audio-player/hooks/use-prewarm';
...
export function WebPlayer() {
    ...
    // Monochroma: warm the YTM fast path for the current + next queued track.
    usePrewarm(player1Url);
    usePrewarm(player2Url);
    ...
    const handlePlayer1Start = useCallback(
        async (player: ReactPlayer) => {
            ...
            const internal = player.getInternalPlayer() as
                | (HTMLMediaElement & { __feishinMediaSource?: MediaElementAudioSourceNode })
                | undefined;
            if (internal) {
                const { context, gains } = webAudio;
                try {
                    // An HTMLMediaElement can be bound to only ONE
                    // MediaElementSourceNode for its entire lifetime — a second
                    // createMediaElementSource on the same element throws
                    // InvalidStateError. This handler can fire more than once for the
                    // same element before setPlayer1Source flushes (remount, repeated
                    // onStart), so cache the node on the element and reuse it. Without
                    // this the second call throws unhandled and the graph is left
                    // half-wired → no audio until a full reload.
                    let source = internal.__feishinMediaSource;
                    if (!source) {
                        source = context.createMediaElementSource(internal);
                        internal.__feishinMediaSource = source;
                    }
                    source.connect(gains[0]);
                    setPlayer1Source(source);
                } catch (error) {
                    console.error('Error connecting web audio source (player 1)', error);
                }
            }
        },
    ...
    const handlePlayer2Start = useCallback(
        async (player: ReactPlayer) => {
            ...
            if (internal) {
                const { context, gains } = webAudio;
                try {
                    // See handlePlayer1Start: an element can only ever have one
                    // MediaElementSourceNode, so cache and reuse it to avoid a second
                    // createMediaElementSource throwing InvalidStateError.
                    let source = internal.__feishinMediaSource;
                    if (!source) {
                        source = context.createMediaElementSource(internal);
                        internal.__feishinMediaSource = source;
                    ...
                    console.error('Error connecting web audio source (player 2)', error);
                }
            }
        },
