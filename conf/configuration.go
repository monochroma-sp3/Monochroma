...
type configOptions struct {
	...
	Feishin                         feishinOptions      `json:",omitzero"`
...
type feishinOptions struct {
	// Enabled serves the Feishin web build as the default UI (at the site root),
	// keeping Navidrome's own UI at /app for administration.
	Enabled bool
	// Path is the absolute path to Feishin's web build output (its "out/web" dir).
	Path string
	// TransferURL is the full URL of the nd-tools (plim) playlist importer's
	// /transfer endpoint. When set, Feishin's sidebar shows an "Import Playlist"
	// entry that opens a native Feishin page posting directly to this URL.
	TransferURL string
}
...
func setViperDefaults() {
	...
	viper.SetDefault("tidal.hifiapiurl", "http://localhost:8001")
	viper.SetDefault("tidal.countrycode", "US")
	viper.SetDefault("tidal.defaultquality", "LOSSLESS")
	viper.SetDefault("feishin.enabled", false)
	viper.SetDefault("feishin.path", "")
	viper.SetDefault("feishin.transferurl", "")
